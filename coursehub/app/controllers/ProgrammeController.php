...
class ProgrammeController
{
    ...
    public function index(Request $req, Response $res): Response
    {
        ...
        $progs = ($kw || $lv)
            ? $this->module->searchProgrammes($kw, $lv)
            : $this->module->getAllPublishedProgrammes();

        $this->logger->info('Programme list viewed', [
            'search'  => $kw ?: null,
            'level'   => $lv ?: null,
            'results' => count($progs)
        ]);
        ...
        $res->getBody()->write(
            $this->view->renderProgrammeList($progs)
        );
    ...
    public function show(Request $req, Response $res, array $args): Response
    {
        ...
        if (!$p) {
            ...
            $_SESSION['flash_error'] = 'Programme not found.';

            return $res
                ->withHeader('Location', '/programmes')
                ->withStatus(302);
        }
        ...
        foreach ($mods as $m) {
            $m['prog_count'] = count(
                $mm->getProgrammesForModule((int)$m['id'])
            );
        ...
        $res->getBody()->write(
            $this->view->renderProgrammeDetail($p, $byYear, $shared)
        );
    ...
    public function registerInterest(
        ...
        return $res
            ->withHeader(
                'Location',
                '/interest/confirmed?programme=' . urlencode($p['title'])
            )
            ->withStatus(302);
    }
    ...
    public function confirmed(Request $req, Response $res): Response
    {
        ...
        $res->getBody()->write(
            $this->view->renderConfirmed($prog)
        );
    ...
    {
        $email = filter_var(
            trim(((array)$req->getParsedBody())['email'] ?? ''),
            FILTER_SANITIZE_EMAIL
        );
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            return $res
                ->withHeader('Location', '/interest/withdraw')
                ->withStatus(302);
        }
        ...
        if ($n > 0) {
            $this->logger->info('Interest withdrawn via email', [
                'email' => $email,
                'count' => $n
            ]);
        } else {
