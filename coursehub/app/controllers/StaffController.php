...
class StaffController
{
    ...
    public function index(Request $req, Response $res): Response
    {
        ...
        $res->getBody()->write(
            $this->view->renderStaffList($staff)
        );
    ...
    public function loginForm(Request $req, Response $res): Response
    {
        if (!empty($_SESSION['staff_id'])) {
            return $res
                ->withHeader('Location', '/staff/portal')
                ->withStatus(302);
        }
    ...
    public function login(Request $req, Response $res): Response
    {
        ...
        // FIXED: failed login logging
        if (
            !$staff ||
            !password_verify($pass, $staff['password_hash'])
        ) {
            $this->logger->warning('Staff login failed', [
                'email' => $email
            ]);
        ...
        // FIXED: successful login logging
    ...
    public function editProfileForm(
        ...
        $staff = $this->model->getStaffById(
            (int)$_SESSION['staff_id']
        );

        $res->getBody()->write(
            $this->view->renderEditProfile($staff)
        );
    ...
    public function updateProfile(
        ...
        $this->logger->info('Staff profile updated', [
            'id'   => $id,
            'name' => $this->h($data['name'] ?? '')
        ]);
        ...
        return $res
            ->withHeader('Location', '/staff/profile/edit')
            ->withStatus(302);
    }
    ...
    public function updatePassword(
        ...
        // FIXED: failed password verification logging
        if (!password_verify($current, $staff['password_hash'])) {

            $this->logger->warning(
                'Staff password change failed — wrong current password',
                ['id' => $id]
            );
            ...
            // FIXED: pass only the hashed password — never the plaintext value
            $this->model->updatePassword(
                $id,
                password_hash($new, PASSWORD_BCRYPT)
            );

            // FIXED: successful password change logging
            $this->logger->info('Staff password changed', [
                'id' => $id
            ]);
    ...
    public function deleteProfile(
        ...
        $this->logger->info('Staff profile deleted', [
            'id'    => $id,
            'email' => $_SESSION['staff_email'] ?? ''
        ]);
        ...
        $_SESSION['flash_success']
            = 'Your staff profile has been deleted.';
