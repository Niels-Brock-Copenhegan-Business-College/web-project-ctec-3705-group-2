...
class ModuleModule
{
    ...
    public function getModulesByProgrammeId(int $pid): array
    {
        $st = $this->db->prepare(
            'SELECT 
             ...
             LEFT JOIN staff s ON m.module_leader_id = s.id
    ...
    /**
     * Retrieve all modules with leader names, programme counts,
    ...
    public function getModuleById(int $id): array|false
    {
        $st = $this->db->prepare(
            'SELECT 
                ...
                s.office AS leader_office,
                s.photo_url AS leader_photo
